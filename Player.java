 ...
 *
 * Description: <A class that defines a player. It contains the name, 
 * the cards in the hand, the number of points, and points to the next player.
 * The type T extends Card.>
 * 
 ***************************************************************************/
class Player <T extends Card> {
	...
	public Player(String name){
		...
		this.hand = new Hand<T>(); //create an empty hand
		this.points = 0; //set the amount of points to 0

	}

	public void setNext(Player<T> p){
		//set next player
		this.next = p; //identify the next player

	}

	public Player<T> getNext(){
		//return next player
		return this.next; //return the next player
	}

	public boolean hasNext() {
		// whether there is a player after me
		if (this.next == null) { //if this.next points to nothing return false
			return false;
		}
		else {
	...
	public int getPoints(){
		...
		return this.points; //return the points
	}
	...
	public boolean receiveCard(T c){
		...
		// return?
		if (this.hand.indexOf(c) == -1) { //check if the card already exists, if it doesnt add it to hand
			this.hand.addCard(c);
			this.points += c.getPoints();
			return true;
		}
		else { //otherwise return false, denoting that the card couldn't be added
			return false;
		}
	}
	...
	public boolean hasCard(T c){
		...
		if (this.hand.indexOf(c) != -1) { //use the indexOf method to determine the index, if the returned index != -1 the card is in the hand
			return true;
		}
		else { //if the returned index == -1 then the card doesn't exist
			return false;
		}
	}
	...
	public T playCard(int index){
		...
		if ((index > this.hand.numCards()) || (index < 0)) {
			throw new RuntimeException("Invalid Index");
		}
		else {
			T s = this.hand.getCard(index);
	...
	public static void main(String[] args) {
		...
		player1.setNext(player2);
		System.out.println(player1.hasNext());
